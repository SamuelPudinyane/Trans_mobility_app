...
def driver_assignment(request):
    ...
    # EXCLUDE locomotives that are IN_MAINTENANCE or READY_FOR_ACTIVATION
    loc_qs = LocomotiveSpec.objects.filter(
        maintenance_status='OPERATIONAL'
    ).exclude(
        status__in=['IN_MAINTENANCE', 'READY_FOR_ACTIVATION']
    ).exclude(
        locomotive__isnull=True
    ).exclude(
        locomotive__exact=''
    ).order_by('-created_at').values('id','locomotive')
    ...
    for row in loc_qs:
        ...
        if nm and nm not in _seen:
            _seen.add(nm)
            loco_id = row.get('id')
            # Count wagons for this locomotive
            wagon_count = LocomotiveWagonAssignment.objects.filter(locomotive_id=loco_id).count()
            locomotives.append({'id': loco_id, 'name': nm, 'wagon_count': wagon_count})
    ...
    if request.method == 'POST':
        ...
        print(f"DEBUG: POST received with action='{action}'")
        print(f"DEBUG: All POST data: {dict(request.POST)}")

        # Unassign (delete) an assignment
        if action == 'unassign':
            assignment_id = request.POST.get('assignment_id')
            print(f"DEBUG: Unassign action - assignment_id='{assignment_id}'")
            if assignment_id:
                try:
                ...
                if a:
                    ...
                    try:
                        notification_message = f"You have been unassigned from locomotive {a.locomotive.locomotive or '(unnamed)'}."
                        DriverAssignmentNotification.objects.create(
                            assignment=a,
                            driver=a.driver,
                        ...
                        logging.error(f"Failed to create unassign notification: {str(e)}")
                    
                    a.delete()
                    
                    # Check if locomotive has no more drivers assigned
                    remaining_assignments = LocomotiveAssignment.objects.filter(locomotive=loco).count()
                    if remaining_assignments == 0 and loco.status == 'ON_DUTY':
                        # Set locomotive back to AVAILABLE if it was ON_DUTY and has no more drivers
                        loco.status = 'AVAILABLE'
                        loco.save()
                    
                    messages.success(request, 'Assignment removed and driver notified')
                else:
                    messages.error(request, 'Assignment not found')
            else:
                print(f"DEBUG: No assignment_id provided")
                messages.error(request, 'No assignment ID provided')
            return redirect('driver_assignment')
        ...
        if action == 'reassign':
            ...
            # Check if new locomotive is under maintenance
            if new_loco.maintenance_status == 'UNDER_MAINTENANCE':
                messages.error(request, f'Cannot reassign to {new_loco.locomotive or "this locomotive"}: it is currently under maintenance')
                return redirect('driver_assignment')
            ...
            old_loco = a.locomotive
            old_loco_name = old_loco.locomotive or "(unnamed)"
            
            # Update assignment
            a.locomotive = new_loco
            a.save()
            
            # Update old locomotive status if no more drivers
            remaining_on_old = LocomotiveAssignment.objects.filter(locomotive=old_loco).count()
            if remaining_on_old == 0 and old_loco.status == 'ON_DUTY':
                old_loco.status = 'AVAILABLE'
                old_loco.save()
            
            # Update new locomotive status to ON_DUTY
            if new_loco.status == 'AVAILABLE':
                new_loco.status = 'ON_DUTY'
                new_loco.save()
            
            # Create notification for reassignment
            try:
                notification_message = f"You have been reassigned from locomotive {old_loco_name} to locomotive {new_loco.locomotive or '(unnamed)'}. Please check your new assignment details."
                DriverAssignmentNotification.objects.create(
                    assignment=a,
                    driver=a.driver,
                ...
                logging.error(f"Failed to create reassign notification: {str(e)}")
            
            messages.success(request, 'Assignment updated and driver notified')
            return redirect('driver_assignment')
        ...
        # Check if locomotive is under maintenance
        if loco.maintenance_status == 'UNDER_MAINTENANCE':
            messages.error(request, f'Cannot assign drivers to {loco.locomotive or "this locomotive"}: it is currently under maintenance')
            return redirect('driver_assignment')

        # Check if locomotive has wagons assigned (OPTIONAL - just show warning)
        from .models import LocomotiveWagonAssignment
        wagon_count = LocomotiveWagonAssignment.objects.filter(locomotive=loco).count()
        if wagon_count == 0:
            messages.warning(request, f'Note: {loco.locomotive or "this locomotive"} has no wagons assigned. You can still assign drivers, but consider assigning wagons first.')
        ...
        for did in selected_driver_ids:
            ...
            if created_flag:
                ...
                try:
                    notification_message = f"You have been assigned to locomotive {loco.locomotive or '(unnamed)'}. Please check your assignment details."
                    DriverAssignmentNotification.objects.create(
                        assignment=obj,
                        driver=driver,
                    ...
                    logging.error(f"Failed to create notification for driver {driver.email}: {str(e)}")

        if created:
            # Update locomotive status to ON_DUTY
            loco.status = 'ON_DUTY'
            loco.save()
            messages.success(request, f'Assigned {created} driver(s) to {loco.locomotive or "(unnamed)"} and sent notifications')
        else:
...
def locomotive_wagon_assignment(request):
    ...
    # Get all operational locomotives
    locomotives = LocomotiveSpec.objects.filter(
        ...
        locomotive__exact=''
    ).order_by('locomotive').values('id', 'locomotive')
    
    # Get only UNASSIGNED wagons (active, operational, and not currently assigned to any locomotive)
    wagons = WagonSpec.objects.filter(
        is_active=True,
        status='AVAILABLE',
        is_assigned=False
    ).order_by('wagon_number').values('id', 'wagon_number', 'wagon_type', 'payload_capacity')
    ...
    if request.method == 'POST':
        ...
        if action == 'assign':
            ...
            if assignment_id:
                try:
                    assignment = LocomotiveWagonAssignment.objects.filter(id=int(assignment_id)).first()
                    if assignment:
                        wagon = assignment.wagon
                        # Mark wagon as available again
                    ...
                    logging.error(f"Failed to unassign wagon: {str(e)}")
    ...
    # Calculate locomotive weight summaries with power calculations
    from django.db.models import Count
    loco_summaries = {}
    
    # Get locomotives to analyze (filter if locomotive_filter is set)
    if locomotive_filter:
        locos_to_analyze = LocomotiveSpec.objects.filter(
            maintenance_status='OPERATIONAL',
            id=locomotive_filter
        )
    else:
        locos_to_analyze = LocomotiveSpec.objects.filter(maintenance_status='OPERATIONAL')
    
    for loco in locos_to_analyze:
        ...
        if wagon_count > 0:
            ...
            # Extract tractive effort (in kN)
            try:
                tractive_effort_str = loco.tractive_effort or "0"
                # Remove any units like "kN" and extract number
                import re
                numbers = re.findall(r'[-+]?\d*\.?\d+', tractive_effort_str)
                tractive_effort = float(numbers[0]) if numbers else None
            except (ValueError, TypeError, IndexError):
                tractive_effort = None
            
            # Calculate power requirements and performance metrics
            # Rule of thumb: 1 HP per ton for flat terrain, 3-5 HP per ton for grades
            power_required_flat = total_weight * 1  # HP for flat terrain
            power_required_grade = total_weight * 3.5  # HP for typical grades
            
            # Estimate locomotive power from tractive effort (rough approximation)
            # Power (HP) ≈ Tractive Effort (kN) × Speed (km/h) / 2.65
            # Assume average speed of 60 km/h for freight
            loco_power_hp = (tractive_effort * 60 / 2.65) if tractive_effort else None
            
            # Calculate power-to-weight ratio
            power_to_weight = (loco_power_hp / total_weight) if loco_power_hp and total_weight > 0 else None
            
            # Determine if locomotive can handle the load
            can_handle_status = None
            can_handle_message = None
            if loco_capacity and loco_power_hp:
                capacity_percent = (total_weight / loco_capacity * 100) if loco_capacity > 0 else 0
                
                # Check both weight capacity and power requirements
                if capacity_percent > 100:
                    can_handle_status = 'overweight'
                    can_handle_message = 'OVERWEIGHT - Exceeds capacity'
                elif power_to_weight < 1:
                    can_handle_status = 'underpowered'
                    can_handle_message = 'UNDERPOWERED - May struggle on grades'
                elif capacity_percent > 90:
                    can_handle_status = 'warning'
                    can_handle_message = 'AT LIMIT - Operating at maximum capacity'
                elif power_to_weight < 2:
                    can_handle_status = 'marginal'
                    can_handle_message = 'MARGINAL - Adequate for flat terrain only'
                else:
                    can_handle_status = 'optimal'
                    can_handle_message = 'OPTIMAL - Good power and capacity margins'
            
            loco_summaries[loco.id] = {
                'name': loco.locomotive,
                'wagon_count': wagon_count,
                'wagons_with_cargo': wagons_with_cargo,
                'total_tare_weight': total_tare_weight,
                'total_cargo_weight': total_cargo_weight,
                'total_weight': total_weight,
                'capacity': loco_capacity,
                'capacity_used_percent': (total_weight / loco_capacity * 100) if loco_capacity and loco_capacity > 0 else None,
    ...
    # Get list of locomotives with wagon assignments for filter dropdown
    locomotives_with_assignments = LocomotiveSpec.objects.filter(
        is_active=True,
    ...
    return render(request, 'locomotive_wagon_assignment.html', {
        'Account_type': 'ADMIN',
        'locomotives': locomotives,
        'wagons': wagons,
        'assignments': assignments,
...
def complete_trip(request):
    """Driver view to complete their trip and release locomotive and wagons"""
    account_type = request.session.get('account_type')
    ...
    # Get driver's current locomotive assignments
    assignments = LocomotiveAssignment.objects.filter(driver=request.user).select_related('locomotive')
    ...
    if request.method == 'POST':
        ...
        try:
            assignment = LocomotiveAssignment.objects.filter(
                id=int(assignment_id),
                driver=request.user
            ).first()
            
            if not assignment:
                messages.error(request, 'Assignment not found or you are not authorized')
                return redirect('complete_trip')
            
            locomotive = assignment.locomotive
            
            # Release all wagons assigned to this locomotive
            wagon_assignments = LocomotiveWagonAssignment.objects.filter(locomotive=locomotive)
            released_wagon_count = 0
            for wa in wagon_assignments:
                wagon = wa.wagon
                wagon.is_assigned = False
                wagon.status = 'AVAILABLE'
                wagon.save()
                wa.delete()
                released_wagon_count += 1
            
            # Delete the driver assignment
            assignment.delete()
            
            # Check if locomotive has any other drivers assigned
            remaining_drivers = LocomotiveAssignment.objects.filter(locomotive=locomotive).count()
            if remaining_drivers == 0:
                # No more drivers, set locomotive to AVAILABLE
                locomotive.status = 'AVAILABLE'
                locomotive.save()
                messages.success(request, f'Trip completed! Locomotive {locomotive.locomotive} and {released_wagon_count} wagon(s) are now available for reassignment.')
            else:
                messages.success(request, f'Trip completed! {released_wagon_count} wagon(s) released. Locomotive {locomotive.locomotive} remains ON_DUTY with {remaining_drivers} driver(s).')
            
            return redirect('complete_trip')
            
        except Exception as e:
            logging.error(f"Failed to complete trip: {str(e)}")
            messages.error(request, 'Failed to complete trip. Please try again.')
            return redirect('complete_trip')
    
    # GET request - show driver's assignments
    driver_assignments = []
    for assignment in assignments:
        loco = assignment.locomotive
        wagon_count = LocomotiveWagonAssignment.objects.filter(locomotive=loco).count()
        
        driver_assignments.append({
            'assignment_id': assignment.id,
            'locomotive_name': loco.locomotive,
            'locomotive_id': loco.id,
            'wagon_count': wagon_count,
            'assigned_at': assignment.assigned_at,
        })
    
    return render(request, 'complete_trip.html', {
        'Account_type': 'DRIVER',
...
def map_location(request):
    ...
    # Get parameters from emergency alert redirect
    ...
    context = {
        'Account_type': account_type,
        'highlight_lat': highlight_lat,
        'highlight_lng': highlight_lng,
        'highlight_user': highlight_user,
        'highlight_issue': highlight_issue,
    }
    
    if account_type in ("DRIVER", "ADMIN"):
...
def route_and_node_preference(request):
    ...
    if account_type == "DRIVER":
        assigned_locos = LocomotiveAssignment.objects.filter(driver=request.user).select_related('locomotive')
        assigned_loco_ids = [a.locomotive.id for a in assigned_locos]
        rp_qs = RoutePreferenceSpec.objects.filter(locomotive_id__in=assigned_loco_ids).select_related('locomotive').order_by('-created_at')
    else:  # ADMIN
        assigned_locos = LocomotiveSpec.objects.all()
        assigned_loco_ids = [l.id for l in assigned_locos]
        rp_qs = RoutePreferenceSpec.objects.all().select_related('locomotive').order_by('-created_at')

    prefs = []
    for rp in rp_qs:
        prefs.append({
            'id': rp.id,
            'name': rp.preferred_route_corridor,
            'preferred_route_corridor': rp.preferred_route_corridor,
            'transshipment_required': rp.transshipment_required,
            'delivery_node': rp.delivery_node,
            'receiving_facility': rp.receiving_facility,
            'locomotive_name': rp.locomotive.locomotive if rp.locomotive else '',
        })
    ...
    if request.method == 'POST':
        try:
            ...
            if selected_id:
                try:
                    if account_type == "DRIVER":
                        existing = RoutePreferenceSpec.objects.filter(
                            id=int(selected_id),
                            created_by=request.user,
                            locomotive_id__in=assigned_loco_ids
                        ).first()
    ...
    locomotives = [{'id': a.locomotive.id if hasattr(a, 'locomotive') else a.id, 
                    'name': a.locomotive.locomotive if hasattr(a, 'locomotive') else a.locomotive} 
                   for a in assigned_locos]
    # For GET (or if no POST-related return happened) render the page
    return render(request, 'route_and_node_preference.html', {'Account_type': account_type, 'prefs': prefs, 'locomotives': locomotives})
...
def security_manager(request):
    account_type=request.session.get('account_type')
    return render(request, 'security_manager.html',{'Account_type':"Security Manager"})
...
def maintanance_scheduling(request):
    ...
    if sort_by == 'scheduled_date':
        maintenance_schedules = maintenance_query.order_by('-scheduled_date')
    elif sort_by == 'expected_completion':
        maintenance_schedules = maintenance_query.order_by('expected_completion_date')
    elif sort_by == 'status':
        maintenance_schedules = maintenance_query.order_by('status', '-scheduled_date')
    else:
        maintenance_schedules = maintenance_query.order_by('-scheduled_date')
    
    # Calculate urgency for each maintenance schedule based on last maintenance
    from datetime import timedelta
    from django.utils import timezone
    
    for schedule in maintenance_schedules:
        # Get the item's last completed maintenance
        ...
        if last_maintenance and last_maintenance.actual_completion_date:
            ...
            # Determine urgency level based on months since last maintenance
            if months_since >= 12:
                schedule.urgency_level = 'CRITICAL'
                schedule.urgency_color = '#dc3545'  # Red
            elif months_since >= 9:
                schedule.urgency_level = 'HIGH'
                schedule.urgency_color = '#fd7e14'  # Orange
            elif months_since >= 6:
                schedule.urgency_level = 'MEDIUM'
                schedule.urgency_color = '#ffc107'  # Yellow
            elif months_since >= 3:
                schedule.urgency_level = 'LOW'
                schedule.urgency_color = '#90ee90'  # Light green
            else:
                schedule.urgency_level = 'GOOD'
                schedule.urgency_color = '#28a745'  # Green
            
            schedule.days_since_maintenance = days_since
            schedule.last_maintenance_date = last_maintenance.actual_completion_date
        else:
            # No previous maintenance found
            schedule.urgency_level = 'CRITICAL'
    ...
    # Get available wagons (not in maintenance)
    ...
    # Get statistics
    total_schedules = maintenance_query.count()
    scheduled_count = maintenance_query.filter(status='SCHEDULED').count()
    in_progress_count = maintenance_query.filter(status='IN_PROGRESS').count()
    completed_count = maintenance_query.filter(status='COMPLETED').count()
    ready_for_activation_count = maintenance_query.filter(status='READY_FOR_ACTIVATION').count()
    
    locomotive_maintenance_count = maintenance_query.filter(item_type='LOCOMOTIVE').count()
    wagon_maintenance_count = maintenance_query.filter(item_type='WAGON').count()
...
def wagon_dashboard(request):
    ...
    for assignment in wagon_assignments:
        ...
        if wagon_id not in wagon_assignment_map:
            wagon_assignment_map[wagon_id] = []
        wagon_assignment_map[wagon_id].append({
    ...
    # Add assignment info to in_use_wagons
    for wagon in in_use_wagons:
        wagon.assignments = wagon_assignment_map.get(wagon.id, [])
    
    # Get wagon type distribution
    wagon_types = all_wagons.values('wagon_type').annotate(
        count=Count('id')
    ).order_by('-count')
    
    # Get maintenance schedules for WAGONS ONLY
    from django.utils import timezone
    from datetime import timedelta
    ...
    maintenance_query = MaintenanceSchedule.objects.select_related(
        ...
        item_type='WAGON',  # Only show wagon maintenance
        status__in=['SCHEDULED', 'IN_PROGRESS']
    )
    ...
    # Calculate urgency for each maintenance
    maintenance_list = []
    critical_count = 0
    high_count = 0
    
    for schedule in all_maintenance:
        # Get the last completed maintenance for this wagon
        last_maintenance = MaintenanceSchedule.objects.filter(
            wagon=schedule.wagon,
            status='COMPLETED',
            actual_completion_date__isnull=False
        ...
        if last_maintenance and last_maintenance.actual_completion_date:
            ...
            days_since_last = 999  # Very high number to indicate critical
        
        # Determine urgency based on days since last maintenance
        # Using 12 months (365 days) as critical threshold as requested
        if days_since_last >= 365:
            urgency_level = 'CRITICAL'
            urgency_color = '#dc3545'  # Red
            critical_count += 1
        elif days_since_last >= 270:  # 9-12 months
            urgency_level = 'HIGH'
            urgency_color = '#fd7e14'  # Orange
            high_count += 1
        elif days_since_last >= 180:  # 6-9 months
            urgency_level = 'MEDIUM'
            urgency_color = '#ffc107'  # Yellow
        elif days_since_last >= 90:   # 3-6 months
            urgency_level = 'LOW'
            urgency_color = '#90ee90'  # Light green
        else:  # 0-3 months
            urgency_level = 'GOOD'
    ...
    # Get list of locomotives for filter dropdown
